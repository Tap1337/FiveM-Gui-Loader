...
namespace ImTricks {
	...
	namespace NotifyManager {
		...
		void HandleNotifies(ImDrawList* draw) {
			...
			ImVec2 NotifyPos = ImVec2(ScreenSize.x - 230, 30.f);

			auto DrawNotify = [&draw, &NotifyPos](NotifyStruct notify) {

				const auto NotifyEndPos = NotifyPos + ImVec2(200, 42 + ImGui::CalcTextSize( notify.message ).y );
				...
				draw->AddRectFilled( NotifyPos, NotifyEndPos, ImGui::GetColorU32( ImGuiCol_ChildBg, alpha ), 4 );
				draw->AddRect( NotifyPos, NotifyEndPos, ImColor( 0.f, 0.f, 0.f, alpha ), 4 );
				draw->AddRect( NotifyPos + ImVec2{ 1, 1 }, NotifyEndPos - ImVec2{ 1, 1 }, ImGui::GetColorU32( ImGuiCol_Border, ImClamp( alpha, 0.f, 1.f ) ), 4 );

				auto StateColor = ImColor(45, 45, 45);

				switch (notify.state) {
				case ImTrickNotify_Success: StateColor = ImGui::GetColorU32( ImGuiCol_Scheme, ImClamp( alpha, 0.f, 1.f ) ); break;
				case ImTrickNotify_Warning: StateColor = ImColor( 212 / 255.f, 137 / 255.f, 52 / 255.f, ImClamp( alpha, 0.f, 1.f ) ); break;
				case ImTrickNotify_Danger: StateColor = ImColor( 206 / 255.f, 46 / 255.f, 46 / 255.f, ImClamp( alpha, 0.f, 1.f ) ); break;
				...
				draw->AddRectFilledMultiColor( { NotifyPos.x + ( NotifyEndPos.x - NotifyPos.x ) / 2, NotifyPos.y + 1 }, { NotifyEndPos.x, NotifyPos.y + 2 }, StateColor, ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.f ), ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.f ), StateColor );
				draw->AddRectFilledMultiColor( { NotifyPos.x, NotifyPos.y + 1 }, { NotifyPos.x + ( NotifyEndPos.x - NotifyPos.x ) / 2, NotifyPos.y + 2 }, ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.f ), StateColor, StateColor, ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.f ) );

				draw->AddRectFilledMultiColor( NotifyPos + ImVec2{ 0, 6 }, NotifyPos + ImVec2{ 10, 26 }, ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.1f * ImClamp( alpha, 0.f, 1.f ) ), ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.f ), ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.f ), ImColor( StateColor.Value.x, StateColor.Value.y, StateColor.Value.z, 0.1f * ImClamp( alpha, 0.f, 1.f ) ) );
				draw->AddRectFilled( NotifyPos + ImVec2{ 0, 6 }, NotifyPos + ImVec2{ 1, 26 }, StateColor );
				draw->AddText( ImGui::GetIO( ).Fonts->Fonts[2], ImGui::GetIO( ).Fonts->Fonts[2]->FontSize, { NotifyPos.x + 8, NotifyPos.y + 16 - ImGui::GetIO( ).Fonts->Fonts[2]->FontSize / 2 }, StateColor, notify.state == ImTrickNotify_Success ? "E" : notify.state == ImTrickNotify_Warning ? "F" : "G" );
				draw->AddText( ImGui::GetIO( ).Fonts->Fonts[1], ImGui::GetIO( ).Fonts->Fonts[1]->FontSize, { NotifyPos.x + 14 + ImGui::GetIO( ).Fonts->Fonts[2]->FontSize, NotifyPos.y + 16 - ImGui::GetIO( ).Fonts->Fonts[1]->FontSize / 2 }, ImGui::GetColorU32( ImGuiCol_Text, ImClamp( alpha, 0.f, 1.f ) ), notify.state == ImTrickNotify_Success ? "Notification" : notify.state == ImTrickNotify_Warning ? "Warning" : "Error" );

				draw->AddText( { NotifyPos.x + 12, NotifyPos.y + 32 }, ImGui::GetColorU32( ImGuiCol_TextDisabled, ImClamp( alpha, 0.f, 1.f ) ), notify.message );

				

				NotifyPos.y += 50.f + ImGui::CalcTextSize( notify.message ).y;
			};
