...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    auto hovered_anim = ImTricks::Animations::FastFloatLerp( label, 0, hovered, 0.f, 1.f, ui::anim_speed );
    auto held_anim = ImTricks::Animations::FastFloatLerp( label, 1, held, 0.f, 1.f, ui::anim_speed );

    ui::rect_glow( window->DrawList, bb.Min, bb.Max, GetColorU32( ImGuiCol_Scheme, 0.05f ), 35, 8.f );
    
    ui::render_gradient( bb.Min, bb.Max, GetColorU32( ImGuiCol_Scheme ), GetColorU32( ImGuiCol_SchemeLight ), GetColorU32( ImGuiCol_SchemeLight ), GetColorU32( ImGuiCol_Scheme ), style.FrameRounding );
    window->DrawList->AddRect( bb.Min, bb.Max, ImColor( 1.f, 1.f, 1.f, style.Alpha * ( hovered_anim * 0.3f + held_anim * 0.1f ) ), style.FrameRounding );
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size), inner_rect.Min.y, outer_rect.Max.x, inner_rect.Max.y - 24);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2( square_sz + style.ItemInnerSpacing.x + label_size.x, square_sz ));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd( total_bb, id );
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = CalcItemWidth();
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2( w, style.ItemInnerSpacing.y + g.FontSize * 2 + style.FramePadding.y * 2 ));
    const ImRect bb(total_bb.Min + ImVec2( 0, style.ItemInnerSpacing.y + g.FontSize ), total_bb.Max);
    ItemSize(total_bb, style.FramePadding.y);
    ...
    RenderFrame( bb.Min, bb.Max, GetColorU32( ImGuiCol_FrameBg ), style.FrameRounding );

    RenderText( total_bb.Min, label );

    if ( preview_value )
        RenderText( bb.Min + style.FramePadding, preview_value );
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (Selectable(item_text, item_selected, ImGuiSelectableFlags_NoPadWithHalfSpacing, { 0, 30 }))
        {
            value_changed = true;
            *current_item = i;
        }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2( CalcItemWidth( ), label_size.y + style.ItemInnerSpacing.y + 5 ));
    const ImRect frame_bb(total_bb.Min + ImVec2( 0, label_size.y + style.ItemInnerSpacing.y ), total_bb.Max);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, const char* icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float anim = ImTricks::Animations::FastFloatLerp( label, 0, g.ActiveId == id || hovered || strlen( buf ) > 0, 0.f, 1.f, ui::anim_speed );
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding + ImVec2{ icon ? 24.f : 0.f, 0.f };
    ...
    if ( icon ) window->DrawList->PushClipRect( { frame_bb.Min.x + style.FramePadding.x + 24, frame_bb.Min.y }, frame_bb.Max, 1 );
    ...
    if ( icon ) window->DrawList->PopClipRect( );
    ...
    if ( icon ) {
        ui::circle_glow( { frame_bb.Min.x + style.FramePadding.x + io.Fonts->Fonts[2]->FontSize / 2 - 2, frame_bb.GetCenter( ).y }, 0, GetColorU32( ImGuiCol_Scheme, 0.1f * anim ), 5.f );
        window->DrawList->AddText( io.Fonts->Fonts[2], io.Fonts->Fonts[2]->FontSize, { frame_bb.Min.x + style.FramePadding.x, frame_bb.GetCenter( ).y - io.Fonts->Fonts[2]->FontSize / 2 - 1 }, icon_col, icon );
    }
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoSmallPreview | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;
                SetNextItemWidth(square_sz * 6.0f); // Use 256 + bar sizes?
